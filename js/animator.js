...
function checkInVP(element) {
    bounds = element.getBoundingClientRect()
    return bounds.top >= 0 && bounds.left >= 0 && bounds.bottom <= heightVP;
}
...
interAbout = setInterval(() => headChanger(aboutHead, aboutUnder, interAbout), 300)
interProject = setInterval(() => headChanger(projectHead, projectUnder, interProject), 300)

interskillpy = setInterval(() => skillChanger(skill1, prog1, interskillpy), 300)
interskillboot = setInterval(() => skillChanger(skill2, prog2, interskillboot), 300)
interskillhtml = setInterval(() => skillChanger(skill3, prog3, interskillhtml), 300)
interskillcss = setInterval(() => skillChanger(skill4, prog4, interskillcss), 300)
interskilljs = setInterval(() => skillChanger(skill5, prog5, interskilljs), 300)
